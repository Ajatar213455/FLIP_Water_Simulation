	...
	void transferVelocities(bool toGrid, float flipRatio) {
		...
		if (toGrid) {
			...
			for (int i = 0; i < m_iNumSpheres; i++) {
				...
				int cellPos_x = floor(spherePos[0] * m_fInvSpacing); cellPos_x = max(min(cellPos_x, 0), m_iCellX - 1);
				int cellPos_y = floor(spherePos[1] * m_fInvSpacing); cellPos_y = max(min(cellPos_y, 0), m_iCellY - 1);
				int cellPos_z = floor(spherePos[2] * m_fInvSpacing); cellPos_z = max(min(cellPos_z, 0), m_iCellZ - 1);
				int cellIdx = cellPos_x * n + cellPos_y * m + cellPos_z;
